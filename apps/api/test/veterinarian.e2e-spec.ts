...
describe('Veterinarians (e2e)', () => {
  ...
  describe('GET /veterinarians', () => {
    it('returns an array', async () => {
      const res = await api.get('/veterinarians').expect(200);
  ...
  describe('GET /veterinarians/search', () => {
    it('applies default pagination', async () => {
      const res = await api.get('/veterinarians/search').expect(200);
      expect(Array.isArray(res.body)).toBe(true);
  ...
  describe('CRUD round-trip', () => {
    ...
    it('reads the created veterinarian by id', async () => {
      await api.get(`/veterinarians/${createdId}`).expect(200);
    });
    ...
    it('updates the veterinarian', async () => {
      await api
        ...
        .expect(200);
      const res = await api.get(`/veterinarians/${createdId}`).expect(200);
      expect(res.body.name).toBe('E2E Vet Renamed');
    });

    it('deletes the veterinarian', async () => {
      await api.delete(`/veterinarians/${createdId}`).expect(200);
      await api.get(`/veterinarians/${createdId}`).expect(404);
    });
  });
});
