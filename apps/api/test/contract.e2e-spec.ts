...
describe('Contracts (e2e)', () => {
  ...
  describe('GET /contracts', () => {
    it('returns an array', async () => {
      const res = await api.get('/contracts').expect(200);
  ...
  describe('GET /contracts/search', () => {
    it('applies default pagination', async () => {
      const res = await api.get('/contracts/search').expect(200);
      expect(Array.isArray(res.body)).toBe(true);
      expect(res.body.length).toBeLessThanOrEqual(10);
    });

    it('rejects an invalid contract_category enum', async () => {
      await api.get('/contracts/search?contract_category=Toys').expect(400);
    });

    it('rejects an invalid status enum', async () => {
      await api.get('/contracts/search?status=Paused').expect(400);
    });
  });
  ...
  describe('GET /contracts/:id', () => {
    ...
    it('returns 404 for a missing id (NotFoundException)', async () => {
      await api.get(`/contracts/${MISSING_ID}`).expect(404);
    });
  });
  ...
  describe('PATCH/DELETE missing contract', () => {
    ...
    it('returns 404 when deleting a missing contract', async () => {
      await api.delete(`/contracts/${MISSING_ID}`).expect(404);
    });
  });
  ...
  describe('CRUD round-trip', () => {
    ...
    it('reads the created contract by id', async () => {
      await api.get(`/contracts/${createdId}`).expect(200);
    });
    ...
    it('deletes the contract', async () => {
      await api.delete(`/contracts/${createdId}`).expect(200);
      await api.get(`/contracts/${createdId}`).expect(404);
    });
  });
});
