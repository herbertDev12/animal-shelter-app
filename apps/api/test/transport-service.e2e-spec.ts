...
describe('Transport Services (e2e)', () => {
  ...
  describe('GET /transport-services', () => {
    it('returns an array', async () => {
      const res = await api.get('/transport-services').expect(200);
  ...
  describe('GET /transport-services/search', () => {
    it('applies default pagination', async () => {
      const res = await api.get('/transport-services/search').expect(200);
      expect(Array.isArray(res.body)).toBe(true);
      expect(res.body.length).toBeLessThanOrEqual(10);
    });

    it('rejects an invalid status enum', async () => {
      await api.get('/transport-services/search?status=Paused').expect(400);
    });
  });
  ...
  describe('CRUD round-trip', () => {
    ...
    it('reads the created transport service by id', async () => {
      await api.get(`/transport-services/${createdId}`).expect(200);
    });
    ...
    it('updates the transport service', async () => {
      await api
        ...
        .send({ vehicle: 'Truck' })
        .expect(200);
      const res = await api.get(`/transport-services/${createdId}`).expect(200);
      expect(res.body.vehicle).toBe('Truck');
    });

    it('deletes the transport service', async () => {
      await api.delete(`/transport-services/${createdId}`).expect(200);
      await api.get(`/transport-services/${createdId}`).expect(404);
    });
  });
});
