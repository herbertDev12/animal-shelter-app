...
import { createVeterinarian } from "../services";
...
export function CreateVeterinarianForm({
  ...
  const mutation = useMutation({
    ...
    onError: () => {
      toast.error("Failed to create veterinarian. Please try again.");
    },
  });
  ...
  const onInvalid = (errors: FieldErrors<CreateVeterinarian>) => {
    ...
    toast.error("Couldn't create veterinarian", {
      description:
        messages.length > 0
  ...
  return (
    <div className="bg-[#161a21] rounded-2xl border border-gray-800/50 p-6">
      <h3 className="text-lg font-bold text-white mb-4">Create Veterinarian</h3>
