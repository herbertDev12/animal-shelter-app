...
import type { Clinic } from "@repo/schemas";
import { CustomTable } from "@/components/custom-table";
import { fetchClinics, deleteClinic } from "../services";
...
export function ClinicsList() {
  ...
  const { data: clinics = [], isLoading } = useQuery({
    queryKey: ["clinics", queryFilters],
    queryFn: () => fetchClinics(queryFilters),
  });
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 pt-4">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 mt-1">
            Manage veterinary clinics — create, edit, filter and remove.
          </p>
        </div>
        <Link to="/clinics/new">
      ...
      {isLoading ? (
        ...
        <>
          <CustomTable columns={columns} data={clinics} />
          <div className="flex items-center justify-end gap-2 text-sm text-gray-400">
            <span>
              Showing {clinics.length} · page{" "}
            ...
            <Button
              ...
              disabled={clinics.length < filters.limit}
              className="px-3 py-1 rounded bg-[#10131a] border border-gray-800 hover:bg-[#1f2937] disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Next
            </Button>
